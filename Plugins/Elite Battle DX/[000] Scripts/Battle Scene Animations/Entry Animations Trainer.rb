...
class EliteBattle_BasicTrainerAnimations
  ...
  def initialize(viewport, battletype, foe)
	...
	# plays team noise animation if applicable 
	elsif EliteBattle.can_transition?("teamNoise", foe[0].trainer_type, :Trainer, foe[0].name, foe[0].partyID)
      return self.teamNoise(@viewport, foe[0].trainer_type)
...
#-----------------------------------------------------------------------------
  # displays the animation for the Team Noise logo (can be standalone)
  #-----------------------------------------------------------------------------
  def teamNoise(viewport = nil, trainerid = -1)
	 @viewport = viewport if !@viewport && !viewport.nil?
    @sprites = {} if !@sprites
    @viewport.color = Color.new(0, 0, 0, 0)
    # fades viewport to black
    ...
    @viewport.color.alpha = 255
    bitmaps = [
      "Graphics/EBDX/Transitions/TeamNoise/background",
      "Graphics/EBDX/Transitions/TeamNoise/swirl",
      "Graphics/EBDX/Transitions/TeamNoise/ray0",
      "Graphics/EBDX/Transitions/TeamNoise/ray1",
      "Graphics/EBDX/Transitions/TeamNoise/logo0",
      "Graphics/EBDX/Transitions/TeamNoise/logo1",
      "Graphics/EBDX/Transitions/TeamNoise/ring0",
      "Graphics/EBDX/Transitions/TeamNoise/ring1"
    ]
    # try resolve the bitmaps
    bitmaps = checkForTrainerVariant(bitmaps, GameData::TrainerType.get(trainerid))
    # creates background graphic
...
class SunMoonBattleTransitions
  ...
  def initialize(*args)
	...
	@teamNoise = EliteBattle.can_transition?("teamNoise", @trainertype.id, :Trainer, @trainer.name, @trainer.partyID)
  ...
  def getParameters(trainer)
	...
	# method used to check if battling against a registered teamNoise member
    @teamNoise = EliteBattle.can_transition?("teamNoise", trainer.trainer_type, :Trainer, trainer.name, trainer.partyID)
